 ...
 * 
 */
public class Instagram implements UserAPI, RelationshipAPI, MediaAPI,
		CommentAPI, LikeAPI, TagAPI, LocationAPI, GeographyAPI {
	...
	public List<Media> getSelfFeed() throws InstagramException {
		...
		try {
			JSONArray mediaArray = executeGetMethod(apiURL)
					.getJSONArray("data");
	...
	public Media getMostRecentMedia(String userId) throws InstagramException {
		...
		try {
			JSONArray mediaArray = executeGetMethod(apiURL)
					.getJSONArray("data");
			System.out.println(userId + " has " + mediaArray.length()
					+ " images");
	...
	public List<Media> getLastestRecentMedia(String userId)
		...
		try {
			...
			for (int i = 0; i < mediaArray.length(); i++) {
				mediaList.add(new Media().deserialize(mediaArray
						.getJSONObject(i)));
			}
	...
	public List<Media> getAllMedia(String userId) throws InstagramException {
		...
		try {
			do {
				hasNext = false;
				if (!max_id.equals("")) {
					arg = new NameValuePair[1];
					arg[0] = new NameValuePair();
					arg[0].setName("max_id");
					arg[0].setValue(max_id);
				}
				JSONObject response = null;
				if (arg != null) {
					response = executeGetMethod(apiURL, arg);
				} else {
					response = executeGetMethod(apiURL);
				}
				JSONArray mediaArray = response.getJSONArray("data");
				System.out.println(userId + " has more" + mediaArray.length()
						+ " images");
				for (int i = 0; i < mediaArray.length(); i++) {
					mediaList.add(new Media().deserialize(mediaArray
							.getJSONObject(i)));
				...
				JSONObject pagination = new JSONObject(
						response.getString("pagination"));
				try {
					max_id = pagination.getString("next_max_id");
				} catch (JSONException e) {
					max_id = "";
					arg = null;
					System.out.println(userId + " doesn't have anymore media");
	...
	public List<User> searchUsersByName(String name) throws InstagramException {
		...
		try {
			NameValuePair[] params = new NameValuePair[] { new NameValuePair(
					"q", name) };
			JSONArray users = executeGetMethod(apiURL, params).getJSONArray(
					"data");
	...
	public List<User> getUserFollowedByList(String userId)
			throws InstagramException {
		List<User> userList = new ArrayList<User>();
		String apiURL = String.format(RelationshipAPI.userFollowedByPath,
				userId);
		try {
			JSONArray users = executeGetMethod(apiURL).getJSONArray("data");
			for (int i = 0; i < users.length(); i++) {
	...
	/**
	 * List the users who have requested authenticated user's permission to
	 * follow.
	 * 
	 */
	 ...
	 * @param userId
	 */
	public Relationship getUserRelationship(String userId)
			throws InstagramException {
		Utils.checkForNull(accessToken,
				"This OAuth request requires an accessToken.");

		String apiURL = String.format(RelationshipAPI.userRelationshipPath,
				userId);
		JSONObject response = null;
		try {
	 ...
	 * @param action
	 */
	public void setUserRelationship(String userId, RelationshipAction action)
			throws InstagramException {
	...
	/**
	 * Get information about a media object. If you authenticate with an OAuth
	 * Token, you will receive the user_has_liked key which quickly tells you
	 ...
	 *            required.
	 */
	public List<Media> searchMediaInArea(double latitude, double longitude)
			throws InstagramException {
		List<Media> mediaList = new ArrayList<Media>();
		String apiURL = MediaAPI.mediaSearchPath;
		try {
			NameValuePair[] params = new NameValuePair[] {
					new NameValuePair("lat", latitude + ""),
					new NameValuePair("lng", longitude + "") };
			JSONArray medias = executeGetMethod(apiURL, params).getJSONArray(
					"data");
	 ...
	 * @param mediaId
	 */
	public List<Comment> getCommentsOnMedia(String mediaId)
			throws InstagramException {
	 ...
	 *            mediaId.
	 */
	public void addCommentOnMedia(String mediaId, String comment)
			throws InstagramException {
		// TODO Auto-generated method stub
	 ...
	 * @param commentId
	 */
	public void deleteCommentOnMedia(String mediaId, String commentId)
			throws InstagramException {
		// TODO Auto-generated method stub
	 ...
	 * @param mediaId
	 */
	public List<User> getUsersWhoLikedMedia(String mediaId)
			throws InstagramException {
		List<User> userList = new ArrayList<User>();
	 ...
	 *            tag name
	 */
	public List<Media> getRecentlyTaggedMedia(String name)
			throws InstagramException {
		List<Media> mediaList = new ArrayList<Media>();
	...
	public List<Tag> searchTags(String name) throws InstagramException {
		...
		try {
			NameValuePair[] params = new NameValuePair[] { new NameValuePair(
					"q", name) };
			JSONArray tags = executeGetMethod(apiURL, params).getJSONArray(
					"data");
	 ...
	 *            Location Id
	 */
	public List<Media> getRecentMedia(String locationId)
			throws InstagramException {
		List<Media> mediaList = new ArrayList<Media>();
	 ...
	 *            required.
	 */
	public List<Location> searchLocation(double latitude, double longitude)
			throws InstagramException {
		...
		try {
			NameValuePair[] params = new NameValuePair[] {
					new NameValuePair("lat", latitude + ""),
					new NameValuePair("lng", longitude + "") };
			JSONArray locations = executeGetMethod(apiURL, params)
					.getJSONArray("data");
	 ...
	 * your OAuth client. To backfill photos from the location covered by this
	 * geography, use the media search endpoint.
	 * 
	 * @param geoId
	 *            geography subscription Id
	 */
	public List<Media> getRecentMediaFromGeography(String geoId)
			throws InstagramException {
		List<Media> mediaList = new ArrayList<Media>();
	...
	private JSONObject executeGetMethod(String apiURL)
			throws InstagramException {
		System.out.println(apiURL);
		return executeGetMethod(apiURL, null);
	}

	private JSONObject executeGetMethod(String apiURL, NameValuePair[] params)
			throws InstagramException {
		...
		if (params != null) {
			method.setQueryString((NameValuePair[]) ArrayUtils.addAll(params,
					new NameValuePair[] { authParam }));
		} else {
			method.setQueryString(new NameValuePair[] { authParam });
		}
